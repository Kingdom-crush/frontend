...
class SYS_A(AConfig):
    # 按系统软件的A结构搜索头文件路径和c文件路径
    def searchAs(self,start,othSrc):
        srcFiles = []
        incs = []
        # bsp下所有文件
        if os.path.exists(start + "/bsp"):
            # 返回了bsp下的头文件路径，但是不在此处使用，之后统一添加
            tempSrc, tempInc = self.searchAll(start + "/bsp")
            srcFiles.extend(tempSrc)
        # usr/sys下所有文件
        if os.path.exists(start + "/usr/sys"):
            tempSrc, tempInc = self.searchAll(start + "/usr/sys")
            srcFiles.extend(tempSrc)
        # 所有路径为头文件搜索路径
